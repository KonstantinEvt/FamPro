async function menuFunc3100() {
    loadStandardMainPanel();
    let contactCards = await paintContacts(contacts);
    console.log("Contacts are painted", contactCards);
    let temp = `<br>
        <div style="font-family: 'Times New Roman', serif; font-size: 16px; text-align: center; color: chocolate;text-shadow: -1px -1px 0 #f1f1f1,
        1px -1px 0 #f1f1f1,
        -1px  1px 0 #f1f1f1,
        1px  1px 0 #f1f1f1,
        0 -1px 0 #f1f1f1,
        0  1px 0 #f1f1f1,
        -1px  0 0 #f1f1f1,
        1px  0 0 #f1f1f1;" >${langMatrix.get("destination")}</div>
    ...
    for (let i = 1; i < numOfPart.get(31); i++) {
        temp += `<input type="radio" class="btn-check" name="news-radio" onclick="menuFunc${3100+i*10}()" id="news-radio${i}" autoComplete="off" >
        <label class="btn btn-outline-warning" style="padding-right: 3px; padding-left: 3px; text-align:center; font-size: 14px; color: darkred" for="news-radio${i}">${localMenu.get(3100 + i * 10)} 
...
async function getContacts(state) {
    let i = 0;
    let contactsFresh = [];
    let promise = await fetch(endPoints.get('recipient')+"/contact/get", {
        method: "GET",
        headers: {
...
function submitAddContact() {
    ...
    if (document.getElementById("contactPhoto").files[0] != null) {
        ...
        fetch(endPoints.get('photoContact')+`/saveContactPhoto`, {
            method: 'POST',
            headers: {},
            body: file,
        }).then(async status => {
    ...
    fetch(endPoints.get('recipient')+"/contact/add", {
        method: 'POST',
        headers: {
...
async function loadContactsImage(contacts, defaultPhotos) {
    ...
    for (let j = 0; j < contacts.length; j++) {
        ...
        if (contacts[j].contactPhoto) imjUrl = endPoints.get("contactPhoto")+"/get/" + url[j];
        else if (contacts[j].primePhoto) imjUrl = endPoints.get("contactPhoto")+"/getPrime/" + url[j];
        else {
...
function submitEditContact(externId) {
    ...
    if (document.getElementById("contactPhoto").files[0] != null) {
        ...
        file.append("contactPhoto", contactPhoto);
        file.append("externId", document.getElementById("contactExternId").value)
        fetch(endPoints.get('contactPhoto')+`/saveContactPhoto`, {
            method: 'POST',
            headers: {},
            body: file,
        }).then(async status => {
...
function submitMessage() {
    ...
    if (document.getElementById("newsPhoto").files[0] != null) {
        ...
        fetch(endPoints.get('fileSystem')+`/saveNewsPhoto`, {
            method: 'POST',
            headers: {},
            body: file,
        }).then(async status => {
    ...
    const jsonData = JSON.stringify(formData);
    fetch(endPoints.get ('newsPrivate')+"/sendMessage", {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
function addContactBySearch(a) {
    fetch(endPoints.get ('newsPrivate')+"/contact/addRequest/" + a, {
        method: 'GET',
    }).then(async status => {

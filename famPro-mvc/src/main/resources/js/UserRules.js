...
<span>Связи</span>
<br> Введенные связи родитель-ребенок на несуществующего родителя порождают создание отсутсвующего родителя<br>
Данная запись в базе помечается "Отсутствует в базе" и не является полноценно созданной записью <br>
Связь людей через данную запись будет системой игнорироваться (за исключением случая совпадения обоих родителей, которые отсутствуют в базе) <br>
При введении реальной записи всем веткам, построенным на основании отсутвующей записи будет отправлем запрос на слияние <br>
При отказе на слияние соответствующие поля родителя будут очищены как недостоверные, при соглании - ветки будут объединены.
Администрация рекомендует не держать связи с отсутстующими родителями, а создать соответствующие полноенные записи в базе
`;
 }

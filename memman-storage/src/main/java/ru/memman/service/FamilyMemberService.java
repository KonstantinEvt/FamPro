...
import ru.memman.exceptions.*;
import ru.memman.feign.FamilyConnectionClient;
import ru.memman.mappers.FamilyMemberMapper;
import ru.memman.mappers.FioMapper;
import ru.memman.mappers.OldNamesMapper;
import ru.memman.repository.FamilyMemberRepo;
import ru.memman.repository.MainOtherFioRepository;
import ru.memman.repository.MainStorageRepository;
import ru.memman.repository.OldFioRepository;
import ru.memman.utils.FamilyMemberUtils;
...
public class FamilyMemberService extends FioServiceImp<FamilyMember> {
    ...
    private volatile AtomicLong inBasePersonsCount;
    ...
    public FamilyMemberService(FioMapper fioMapper,
                               ...
                               Map<UUID, Localisation> tempLocalisation, LinkedList<Long> personsCount) {
        super(fioMapper);
    ...
    private void setInBasePeopleCount() {
        this.inBasePersonsCount = new AtomicLong(mainStorageRepository.getInBasePersonsCount());
        personsCount.add(inBasePersonsCount.get());
    }
    ...
    private void changeMemberDtoByGuardStatus(FamilyMemberDto familyMemberDto) {
        ...
        if (roles.contains(UserRole.ADMIN.getNameSSO()) || roles.contains(UserRole.MANAGER.getNameSSO()))
            guardStatus = SecretLevel.CONFIDENTIAL;
        else if (roles.contains(UserRole.LINKED_USER.getNameSSO()))
...
//        check for change
    ...
    public FamilyMemberDto addFamilyMember(FamilyMemberDto familyMemberDto) {
        ...
        if (familyMemberDto.getFirstName() == null
                ...
                || familyMemberDto.getLastName() == null
                || familyMemberDto.getBirthday() == null) {
            formAndSendService.sendNotification(token, Attention.NEGATIVE, null, 0L, Subject.NOT_FULLY, localisation);
        ...
        if (fm.isPresent()) {
            formAndSendService.sendNotification(token, Attention.NEGATIVE, null, fm.get().getId(), Subject.DUPLICATE, localisation);
            ...
            Set<OldFio> existOldFio = oldFioRepository.findAllOldFiosWithFamilyMembers(Set.of(familyMemberDto.getUuid()));
            if (!existOldFio.isEmpty()) {
                formAndSendService.sendNotification(token, Attention.NEGATIVE, null, existOldFio.stream().findFirst().get().getId(), Subject.DUPLICATE_HIDE, localisation);
                throw new Dublicate("Такой человек уже есть в базе. Это его альтернативное имя");
            }
        }
        ...
        addChangingToBase(familyMemberDto, familyMember, changing);
        if (!children.isEmpty())
            addChangesInParensInfo(children, familyMember, listToFamily);
        ...
        personsCount.add(inBasePersonsCount.incrementAndGet());
        return result;
    }
    ...
    public FamilyMemberDto updateFamilyMember(FamilyMemberDto familyMemberDto) {
        ...
        Changing changing = new Changing();
        if (dtoId != null) {
            fm = mainStorageRepository.findMemberWithInfoById(dtoId).orElseThrow(() ->
...
//                addChangesInParensInfo(fm.getChilds(), fm, fromTemp.getUuid());
...
//    }
    ...
    @Transactional
    public void addChangesInParensInfo(Set<FamilyMember> setOfChilds, FamilyMember fm, List<FamilyDirective> toFamily) {
        ...
        if (fm.getSex() == Sex.MALE) {
            for (FamilyMember child : setOfChilds) {
                ...
                toFamily.add(getChildDirective(child));
            }
    ...
    public FamilyDirective getChildDirective(FamilyMember child) {
        return FamilyDirective.builder()
    ...
    @Transactional
    public void addChangingToBase(FamilyMemberDto familyMemberDto, FamilyMember familyMember, Changing changing) {
            ...
            if (directive.getSwitchPosition() == null) {
                switch (directive.getCheckStatus()) {
                    ...
                    case LINKED -> {
                        ...
                        if (directive.getTokenUser() != null) {
                            familyMember.setCreator(directive.getTokenUser());

...
@FeignClient(name = "MEMMAN-FAMILY", configuration = FeignRequestIntercepter.class, fallbackFactory = FamilyMemberClient.FamilyMemberFallbackFactory.class)
public interface FamilyMemberClient {

    @PostMapping("/family/members/firstCreator")
    Collection<FamilyMemberDto> getMembersByFirstCreator();

    @PostMapping("/family/members/familyTree/{uuid}/{choice}")

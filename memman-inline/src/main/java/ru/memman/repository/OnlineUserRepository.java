...
public class OnlineUserRepository {
    ...
    public long getUsersCount() {
        long usersInBase;
        try {
            usersInBase = (long) entityManager.createNativeQuery("SELECT count(*) from memman_inline.user_online")
                    .getSingleResult();
        } catch (NoResultException e) {
            log.warn("users in base not register yet ");
            usersInBase = 0;
        }
        return usersInBase;
    }
    @Transactional(readOnly = true)
    public List<BaseUser> getOnlineUsersByExternUuids(List<String> externUuids) {
        List<BaseUser> usersOnline;
        try {
            usersOnline = new ArrayList<> (entityManager.createQuery("from BaseUser bu where bu.externUuid IN :uuids", BaseUser.class)
                    .setParameter("uuids", externUuids)
                    .getResultList());
        } catch (NoResultException e) {
            log.warn("users {} is not register in base yet ", externUuids);

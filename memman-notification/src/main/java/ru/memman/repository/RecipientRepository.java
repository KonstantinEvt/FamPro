...
public class RecipientRepository {
    ...
    @Transactional(readOnly = true)
    public Optional<Recipient> getRecipientByExternId(String externId) {
        Optional<Recipient> recipient;
        try {
            recipient = Optional.of(entityManager.createQuery("from Recipient r where r.externUuid=:externId", Recipient.class)
                    .setParameter("externId", externId)
                    .getSingleResult());
        } catch (NoResultException e) {
            log.warn("recipient with id: {} not found", externId);
            recipient = Optional.empty();
        }
        return recipient;
    }
